...
if [ $DCV_SERVER_UP != "active" ]; then
    echo "TACC:"
    echo "TACC: ERROR - could not confirm dcvserver active, 'systemctl is-active dcvserver' returned '$DCV_SERVER_UP'"
    exit 1
fi
...
echo "TACC: local (compute node) DCV port is $LOCAL_PORT"
...
# wait a brief moment so dcv can clean up after itself
sleep 1
