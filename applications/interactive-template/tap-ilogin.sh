...
# Port forwarding is set up for up to four login nodes.
#
...
for i in `seq 4`; do
  ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 -q -f -g -N -R ${LOGIN_PORT}:${NODE_HOSTNAME_PREFIX}:${LOCAL_PORT} ilogin${i}
done
